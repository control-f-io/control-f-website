...
ship broken. 154 files at this level, this one included, no shared library, no
...
| --- | --- | --- |
| 136 | `check-*.py` — one design-system invariant each, exit 0 or exit 1 | `design-system.yml` on every push, one enumerated step per check; `routine-merge.yml` on every routine branch, by glob |
