...
ship broken. 186 files at this level, this one included, no package, no
...
are exactly two dependencies in the whole directory — Pillow, in `sync-news-notion.py` and
...
| --- | --- | --- |
| 166 | `check-*.py` — one design-system invariant each, exit 0 or exit 1 | `design-system.yml` on every push, one enumerated step per check; `routine-merge.yml` on every routine branch, by glob |
...
picture actually says. `check-glass-floor.py` is the fifth and the one that
reads *composited pixels*: the 46 % bearing tint's contrast floor is arithmetic
and `check-contrast.py` recomputes it, but the 30 % and 4 % tints are promises
about a **named** backdrop that is a drawing, so no file states its colour and
no arithmetic produces the figure. It screenshots each surface with its own
content hidden and hit-tests every sample point with `elementFromPoint` —
because a screenshot of an element's box catches everything painted *over* it
while `backdrop-filter` samples only what is painted *under* it, and reporting
the navigation bar as a card's backdrop is how a gate stops being believed. It
is the slowest check in the directory at about six minutes.
